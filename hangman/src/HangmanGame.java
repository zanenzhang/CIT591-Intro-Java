...
import hangman.HangmanOriginal;

import java.util.ArrayList;

/**
 * Main class that contains the main method and starts by creating a new game of Hangman.
...
public class HangmanGame {
	...
	public static void main(String[] args) {
		...
		while (gameOngoing) {
			...
			//If there are prior incorrect guesses, the display wil show the letters that the human player has incorrectly guessed.
			...
			//If the user guess is not a lower case letter, the game will keep asking the human player for input until it is acceptable.
			char userGuess = 0;
			...
			//Display if the letter is in the Hangman word.
			if (guessCheck == 0) {
				
				System.out.println("Awesome! Great guess, the word has your letter.");
				System.out.println("");
				
			//Display is the letter was already guessed.	
			} else if (guessCheck == 1) {
				
				System.out.println("You already guessed that letter!");
				System.out.println("");
			
			//Display if the letter is not in the Hangman word.
			} else {
	...
	private static char takeUserInput(Scanner scanner) {
		...
		//The human player must enter a lower case letter as a guess, otherwise the game will continue to ask for input.
		while (gettingUserInput) {
